...
export default function FlashcardsHubPanel() {
  const { user } = useAuth();
  ...
  return (
    <div className="fh-page">
      ...
      <style jsx>{`
        .fh-page { padding: 0; max-width: 100%; animation: fadeIn .4s ease; }
        .fh-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 32px; gap: 16px; flex-wrap: wrap; }
        .fh-title { font-size: 1.7rem; font-weight: 900; letter-spacing: -1px; color: var(--text); margin-bottom: 4px; }
        ...
        /* STACKED DECK UI */
        .fh-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 32px 24px; padding-top: 4px; width: 100%; }
        ...
        /* Modals */
        .fh-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.75); backdrop-filter: blur(10px); display: flex; align-items: flex-start; justify-content: center; z-index: 5500; padding: 40px 20px; overflow-y: auto; animation: fadeIn .2s; }
        ...
        @media (max-width: 768px) {
          .fh-page { padding: 0; overflow-x: hidden; }
